...
#include "MapButton.h"
#include "SpectateButton.h"
#include "SpectButton.h"
#include "SpectToggleButton.h"
#include "TeamButton.h"
#include "TeamOnlyCommandButton.h"
#include "ToggleCommandButton.h"
#include "CMenuHandler_PopupSubMenuInput.h"
#include "CMenuHandler_SpectateFollow.h"
...
TeamFortressViewport::TeamFortressViewport(int x, int y, int wide, int tall) : Panel(x, y, wide, tall), m_SchemeManager(wide, tall)
{
    ...
    m_SpectatorCameraMenu = CreateCommandMenu("spectcammenu.txt", 1, PANEL_HEIGHT, true, XRES(200), BUTTON_SIZE_Y / 2, ScreenWidth - (XRES(200) + 15));
    // above bottom bar, flat design
...
//-----------------------------------------------------------------------------
int TeamFortressViewport::CreateCommandMenu(const char* menuFile, int direction, int yOffset, bool flatDesign, float flButtonSizeX, float flButtonSizeY,
    int xOffset)
{
    // COMMAND MENU
    ...
    m_pCommandMenus[newIndex] = new CCommandMenu(NULL, direction, xOffset, yOffset, flButtonSizeX, 300);
    // This will be resized once we know how many items are in it
...
CommandButton* TeamFortressViewport::CreateCustomButton(char* pButtonText, char* pButtonName, int iYOffset)
{
    ...
    if (!strcmp(pButtonName, "!CHANGETEAM"))
    {
        ...
        // Spectate button
        m_pTeamButtons[5] = new SpectateButton(CHudTextMessage::BufferedLocaliseTextString("#Menu_Spectate"), 0, BUTTON_SIZE_Y, CMENU_SIZE_X, BUTTON_SIZE_Y,
            false);
