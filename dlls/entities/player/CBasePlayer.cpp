...
#include "entities/player/CBloodSplat.h"
#include "entities/player/CSprayCan.h"
#include "trains.h"
...
TYPEDESCRIPTION CBasePlayer::m_playerSaveData[] =
{
    ...
    DEFINE_FIELD(CBasePlayer, m_fGameHUDInitialized, FIELD_INTEGER),
    // only used in multiplayer games //AJH Not true anymore, used in g-conts modified camera code as it can draw the hud.
...
void CBasePlayer::WaterMove()
{
    ...
    if (!RANDOM_LONG(0, 0x1f) && RANDOM_LONG(0,AIRTIME - 1) >= air)
    {
        ...
        {
        case 0: EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim1.wav", 0.8, ATTN_NORM);
            break;
        case 1: EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim2.wav", 0.8, ATTN_NORM);
            break;
        case 2: EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim3.wav", 0.8, ATTN_NORM);
            break;
        case 3: EMIT_SOUND(ENT(pev), CHAN_BODY, "player/pl_swim4.wav", 0.8, ATTN_NORM);
            break;
        }
    }
...
CBaseEntity* FindEntityForward(CBaseEntity* pMe)
{
    ...
    UTIL_TraceLine(pMe->pev->origin + pMe->pev->view_ofs, pMe->pev->origin + pMe->pev->view_ofs + gpGlobals->v_forward * 8192, dont_ignore_monsters,
                   pMe->edict(), &tr);
...
void CBasePlayer::CheatImpulseCommands(int iImpulse)
{
    ...
    case 106:
        ...
        if (pEntity)
        {
            ...
            ALERT(at_debug, "pev->renderfx: %d, pev->rendercolor: %3f %3f %3f\n", pEntity->pev->renderfx, pEntity->pev->rendercolor.x,
                  pEntity->pev->rendercolor.y, pEntity->pev->rendercolor.z);
