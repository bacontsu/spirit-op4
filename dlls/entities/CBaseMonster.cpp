    ...
    for (i = 0; i < WorldGraph.m_cNodes; i++)
    {
        ...
        if (flDist >= flMinDist && flDist < flMaxDist)
        {
            ...
            if (tr.flFraction != 1.0)
            {
                ...
                if ((iMyNode == iThreatNode) || WorldGraph.PathLength(iMyNode, nodeNumber, iMyHullIndex, m_afCapability) <= WorldGraph.PathLength(
                    iThreatNode, nodeNumber, iMyHullIndex, m_afCapability))
                {
