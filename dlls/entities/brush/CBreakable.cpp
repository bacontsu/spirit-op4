...
int CBreakable::Classify()
{
    return m_iClass;
}

int CBreakable::ObjectCaps()
{
    return (CBaseEntity::ObjectCaps() & ~FCAP_ACROSS_TRANSITION);
}

BOOL CBreakable::Explodable()
{
    return ExplosionMagnitude() > 0;
}

int CBreakable::ExplosionMagnitude()
{
    return pev->impulse;
}

void CBreakable::ExplosionSetMagnitude(int magnitude)
{
...
void CBreakable::TraceAttack(entvars_t* pevAttacker, float flDamage, Vector vecDir, TraceResult* ptr,
                             int bitsDamageType)
{
        ...
        case matComputer:
            {
                ...
                {
                case 0: EMIT_SOUND(ENT(pev), CHAN_VOICE, "buttons/spark5.wav", flVolume, ATTN_NORM);
                    break;
                case 1: EMIT_SOUND(ENT(pev), CHAN_VOICE, "buttons/spark6.wav", flVolume, ATTN_NORM);
                    break;
                }
            }
            break;
